...
class BA_LRP(InMemoryDataset):
    ...
    def gen_class1(self):
        ...
        data = Data(
            x=x, edge_index=edge_index, y=torch.tensor([[0]], dtype=torch.float)
        )
        ...
        for i in range(2, 20):
            ...
            deg = torch.stack(
                [
                    (data.edge_index[0] == node_idx).float().sum()
                    for node_idx in range(i)
                ],
                dim=0,
            )
            ...
            data.edge_index = torch.cat(
                [
                    data.edge_index,
                    torch.tensor([[node_pick, i], [i, node_pick]], dtype=torch.long),
                ],
                dim=1,
            )
    ...
    def gen_class2(self):
        ...
        edge_index = torch.tensor([[0, 1], [1, 0]], dtype=torch.long)
        data = Data(
            x=x, edge_index=edge_index, y=torch.tensor([[1]], dtype=torch.float)
        )
        epsilon = 1e-30
        ...
        for i in range(2, 20):
            ...
            deg_reciprocal = torch.stack(
                [
                    1 / ((data.edge_index[0] == node_idx).float().sum() + epsilon)
                    for node_idx in range(i)
                ],
                dim=0,
            )
            ...
            for _ in range(1 if i % 5 != 4 else 2):
                ...
                data.edge_index = torch.cat(
                    [
                        data.edge_index,
                        torch.tensor(
                            [[node_pick, i], [i, node_pick]], dtype=torch.long
                        ),
                    ],
                    dim=1,
                )
...
def load_syn_data(dataset_dir, dataset_name, transform=None):
    ...
    if dataset_name.lower() == "BA_2Motifs".lower():
        dataset = BA2MotifDataset(
            root=dataset_dir, name=dataset_name, transform=transform
        )
...
def load_SeniGraph(dataset_dir, dataset_name, transform=None):
    ...
    dataset = SentiGraphDataset(
        root=dataset_dir, name=dataset_name, transform=sent_transform
    )
    return dataset
...
def get_dataloader(
    ...
    batch_size,
    degree_bias=False,
    data_split_ratio=[0.1, 0.025, 0.025],
    seed=0,
):
    """
    Args:
    ...
    if degree_bias:
        ...
        num_test = int(data_split_ratio[2] * len(dataset))
        num_last = len(dataset) - num_train - num_eval - num_test

        train, eval, test, _ = random_split(
            dataset,
            lengths=[num_train, num_eval, num_test, num_last],
