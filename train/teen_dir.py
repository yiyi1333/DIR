...
if __name__ == "__main__":
    ...
    parser.add_argument('--epoch', default=10, type=int, help='training iterations')
    ...
    parser.add_argument('--net_lr', default=2e-6, type=float, help='learning rate for the predictor')
    ...
    experiment_name = f'teen-graph.{args.type}.{args.reg}.{args.commit}.netlr_{args.net_lr}.batch_{args.batch_size}'\
    ...
    for seed in args.seed:
        ...
        for epoch in range(args.epoch):
            ...
            with torch.no_grad():
                ...
                train_acc, train_pre, train_rec, train_f1 = test_metrics(train_loader, att_net, g)
                val_acc, val_pre, val_rec, val_f1 = test_metrics(val_loader, att_net, g)
                test_acc, test_pre, test_rec, test_f1 = test_metrics(test_loader, att_net, g)
                # testing acc
    ...
    logger.info("Test ACC:{:.4f}±{:.4f}  Train ACC:{:.4f}±{:.4f}  Val ACC:{:.4f}±{:.4f}".format(
                    torch.tensor(all_info['test_acc']).mean(), torch.tensor(all_info['test_acc']).std(),
    ...
    logger.info("Test Pre:{:.4f}±{:.4f}  Train Pre:{:.4f}±{:.4f}  Val Pre:{:.4f}±{:.4f}".format(
                    torch.tensor(all_info['test_pre']).mean(), torch.tensor(all_info['test_pre']).std(),
                    torch.tensor(all_info['train_pre']).mean(), torch.tensor(all_info['train_pre']).std(),
                    torch.tensor(all_info['val_pre']).mean(), torch.tensor(all_info['val_pre']).std()
                ))
    logger.info("Test Recall:{:.4f}±{:.4f}  Train Recall:{:.4f}±{:.4f}  Val Recall:{:.4f}±{:.4f}".format(
                    torch.tensor(all_info['test_rec']).mean(), torch.tensor(all_info['test_rec']).std(),
                    torch.tensor(all_info['train_rec']).mean(), torch.tensor(all_info['train_rec']).std(),
                    torch.tensor(all_info['val_rec']).mean(), torch.tensor(all_info['val_rec']).std()
                ))
    logger.info("Test F1:{:.4f}±{:.4f}  Train F1:{:.4f}±{:.4f}  Val F1:{:.4f}±{:.4f}".format(
                    torch.tensor(all_info['test_f1']).mean(), torch.tensor(all_info['test_f1']).std(),
                    torch.tensor(all_info['train_f1']).mean(), torch.tensor(all_info['train_f1']).std(),
                    torch.tensor(all_info['val_f1']).mean(), torch.tensor(all_info['val_f1']).std()
                ))
