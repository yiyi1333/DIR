...
class GCN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, num_classes, dropout_prob=0.25):
...
class GAT(torch.nn.Module):
    def __init__(
        self, in_channels, hidden_channels, num_classes, dropout_prob=0.25, heads=4
    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epoch", default=200, type=int, help="training iterations")
    ...
    dataset = get_dataset(
        dataset_dir="../data/", dataset_name="Graph-Twitter", task=None
    )
    dataloader = get_dataloader(dataset, batch_size=args.batch_size, degree_bias=True)
    ...
    for seed in args.seed:
        ...
        # model = GCN(args.input_channels, args.channels, num_classes, args.dropout).to(device)
        # model = GAT(args.input_channels, args.channels, num_classes, args.dropout).to(device)
        # model = ResGatedGCN(args.input_channels, args.channels, num_classes, args.dropout).to(device)
        # model = AntiSymmetricDGN(args.input_channels, args.channels, num_classes).to(
        #     device
        # )
        # model = GINNet(args.input_channels, args.channels, num_classes, args.dropout).to(device)
        # model = PDNNet(args.input_channels, args.channels, num_classes, args.dropout).to(device)
        # model = GENNet(args.input_channels, args.channels, num_classes, args.dropout).to(device)
        model = PMPGNN(
            args.input_channels,
            args.channels,
            num_classes,
        ...
        model_optimizer = torch.optim.Adam(
            model.parameters(), lr=args.net_lr, weight_decay=1e-3
        )
